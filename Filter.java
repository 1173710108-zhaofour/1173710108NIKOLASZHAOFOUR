...
public class Filter {
	...
	public static List<Tweet> writtenBy(List<Tweet> tweets, String username) {
		List<Tweet> particularUserList = new ArrayList<Tweet>();
		for (int i = 0; i < tweets.size(); i++) {
			if (tweets.get(i).getAuthor().equals(username))
				particularUserList.add(tweets.get(i));
		}
		return particularUserList;
	...
	public static List<Tweet> inTimespan(List<Tweet> tweets, Timespan timespan) {
		List<Tweet> particularTimespanList = new ArrayList<Tweet>();
		for (int i = 0; i < tweets.size(); i++) {
			if (tweets.get(i).getTimestamp().isAfter(timespan.getStart())
					&& tweets.get(i).getTimestamp().isBefore(timespan.getEnd()))
				particularTimespanList.add(tweets.get(i));
		}
		return particularTimespanList;
		// throw new RuntimeException("not implemented");
	}
	...
	public static List<Tweet> containing(List<Tweet> tweets, List<String> words) {
		List<Tweet> containCertainWordsList = new ArrayList<Tweet>();
		for (int i = 0; i < tweets.size(); i++) {
			String newString = new String(tweets.get(i).getText().toLowerCase());
			for (int j = 0; j < words.size(); j++) {
				if (newString.indexOf(words.get(j)) >= 0 && newString.indexOf(words.get(j)) < newString.length()) {
					containCertainWordsList.add(tweets.get(i));
					break;
				}
			}
		}
		return containCertainWordsList;
		// throw new RuntimeException("not implemented");
	}
